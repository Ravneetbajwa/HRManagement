...
public class MarkAttendance extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        for (int x = 0; x < total_employee; x++) {
         ...
         PreparedStatement pstmt = conn.prepareStatement("insert into emp_attendance values(?,?,?,?)");
         pstmt.setInt(1,gs.getAttendanceNumber());
         pstmt.setString(2,employee_code[x]);
         pstmt.setString(3,attendance_date);
         pstmt.setString(4,Attendance_Status);
         pstmt.executeUpdate();
         conn.close();
                     }
             catch(Exception ex)
             {
                 out.print("exception on mark Attendence = "+ex);
             }
